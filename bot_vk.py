        ...
        if event.type == VkEventType.MESSAGE_NEW:
            if event.to_me:
                ...
                if user_request == "начать": 
                    ...
                    write_msg(event.user_id, f"{count} {candidates_to_view[x]}", attachment=photos_to_view, keyboard=temp_kb)                    
                ...
                # при нажатии на кнопку "следующий" помечаем флаг has_seen
                    ...
                    if x == len(candidates_to_view) - 1:                        
                        ...
                        count += 1
                        x += 1 
                        photos_to_show = db.get_photos(candidates_to_view[x][0])                  
                        write_msg(event.user_id, f"{count} {candidates_to_view[x]}", attachment=photos_to_show)   
                    ...
                    for favourite in favourites_to_view:
                        write_msg(event.user_id, f"{favourite}")
                    
                elif user_request == "завершить работу": 
                    temp_kb = generate_keyboard('Запуск')
                    write_msg(event.user_id, "Пока!", keyboard=temp_kb)
                    break
                else:
